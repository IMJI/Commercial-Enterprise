...
class NumberRange {
    ...
    public static fromString(str: string): NumberRange {
        ...
        if (split[0] !== '' && split[1] !== '') {
            let from: number = convertToNumber(split[0]);
            let to: number = convertToNumber(split[1]);
    ...
    public toString(): string {
        if (this._type === NumberRangeType.range)
            return this._from + ':' + this._to;
        else if (this._type === NumberRangeType.from)
            return this._from + ':';
        else if (this._type === NumberRangeType.to)
            return ':' + this._to;
    }
}
