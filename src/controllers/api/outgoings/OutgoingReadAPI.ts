...
class OutgoingReadAPI implements IReadAPI<Outgoing> {
	...
	private async getQueryBuilder(query: ParsedOutgoingQuery): Promise<SelectQueryBuilder<Outgoing>> {
		let queryBuilder = Database.dataSource
			.getRepository(Outgoing)
			.createQueryBuilder('outgoing')
			.leftJoinAndSelect('outgoing.tax', 'tax')
			.leftJoinAndSelect('outgoing.manager', 'manager')
	...
	public async readOne(id: number): Promise<Outgoing> {
	...
	public async read(query: ParsedOutgoingQuery): Promise<Outgoing[]> {
		const queryBuilder = await this.getQueryBuilder(query);
		const data = queryBuilder.getMany();
		return data;
	}

	public async count(query: ParsedOutgoingQuery): Promise<number> {
		const queryBuilder = await this.getQueryBuilder(query);
		const data = queryBuilder.getCount();
		return data;
	}
}

export default new OutgoingReadAPI();
