...
> extends ServiceController<T> {
	...
	public async get(
		...
		try {
			...
			if (!parentId)
				throw new EntityIsNotSpecifiedException(`Parent id is not specified`);
			const timestamp = req.params.timestamp;
			if (timestamp === 'latest') {
				const result = await (this.service as TimestampService<T>).findLatest(
					parentId
				);
				...
				const result = await (this.service as TimestampService<T>).findActualOn(
					parentId,
					date
				);
				if (result) res.status(200).json(result);
				...
				const result = await (this.service as TimestampService<T>).findAll(
					parentId
				);
